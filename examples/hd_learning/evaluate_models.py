...
DIMENSIONS = 10000
DATASETS = [
    ds.PittsburgBridgesType,
    ds.AcuteInflammation,
    ds.AcuteNephritis,
    ds.Balloons,
    ds.BreastCancer,
    ds.BreastCancerWiscProg,
    ds.ConnBenchSonarMinesRocks,
    ds.Echocardiogram,
    ds.Fertility,
    ds.Hepatitis,
    ds.IlpdIndianLiver,
    ds.Ionosphere,
    ds.MolecBiolPromoter,
    ds.Monks1,
    ds.Monks2,
]
...
def load_train_test(dataset):
    ...
    train, test = torch.utils.data.random_split(
        d, [train_size, test_size], generator=torch.Generator().manual_seed(42)
    )
...
def hd_learn(model, encode, train_loader, test_loader, num_classes, learn):
    ...
    with torch.no_grad():
        for i in range(1):
            if "accuracy" in METRICS:
                ...
                accuracy_train2 = torchmetrics.Accuracy(
                    "multiclass", num_classes=num_classes
                )
            if "recall" in METRICS:
            ...
            print(
                "Train",
                (model.train_accuracy.compute().item() * 100),
                (model.train_accuracy_base.compute().item() * 100),
            )
            ...
            for samples, labels in tqdm(test_loader, desc="Testing", disable=True):
                ...
                if learn == "add_combined":
                    combined = True
                    logit, logit_base = model_test.forward2(
                        samples_hv, dot=True, combined=combined, test=True
                    )
                    pred_base = logit_base.argmax(1)
                else:
                    logit = model_test(
    ...
    if learn == "add_combined":
        ...
        sim2 = abs(
            torchhd.cos_similarity(model.weight_base[0], model.weight_base[1])
        ).item()
        print(
            learn,
            (accuracy_test.compute().item() * 100),
            (accuracy_test2.compute().item() * 100),
            sim1,
            sim2,
            sim1 > sim2,
        )
        ...
        print(
            learn,
            (accuracy_test.compute().item() * 100),
            sum_sim.item() / len(train_loader),
            sum_sim_diff / len(train_loader),
        )
