...
df.groupby(["Name", var])[acc].mean().unstack().reindex(methods_order, axis=1).plot(
    kind="bar", ax=ax, width=0.8
)

print(df.groupby(["Name", var])[acc].mean().unstack().reindex(methods_order, axis=1))
...
fig, ax = plt.subplots(figsize=(10, 6))
df.groupby(["Name", var])["Time"].mean().unstack().reindex(methods_order, axis=1).plot(
    kind="bar", ax=ax, width=0.8
)
ax.set_xlabel(var + " and Dataset Name")
ax.set_ylabel("Time")
