...
for i, encoding in enumerate(encodings):
    ...
    grouped_data = encoding_data.groupby(['Name', 'Method'])['Accuracy'].agg(['mean', 'var'])

    # Reshape the data to a pivot table
    pivot_data = grouped_data.unstack()

    # Create bar plot with error bars
    pivot_data['mean'].plot(ax=axs[i//3][i%3], kind='bar', yerr=pivot_data['var'], capsize=3)
    axs[i//3][i%3].set_title(f'Accuracy by Name and Method (Encoding: {encoding}, Retrain: False)')
