...
print()
df22 = pd.DataFrame(
    data[data[col] == "HashmapProjectionOnlineV2"]["Accuracy"]
).reset_index()["Accuracy"]
new_df["V2"] = df22
# print(new_df)
...
for idx, i in enumerate(splits):
    ...
    for l in range(len(rows)):
        for j in range(len(cols)):
            text = ax.text(
                l, j, round(heat[j, l], 2), ha="center", va="center", color="w"
            )
    ...
    fig.subplots_adjust(
        top=0.981, bottom=0.049, left=0.05, right=0.981, hspace=0.2, wspace=0.2
    )
    ...
    ax.set_title(title + " part " + str(idx))
    fig.subplots_adjust(
        top=0.981, bottom=0.2, left=0.05, right=0.981, hspace=0.2, wspace=0.2
    )
