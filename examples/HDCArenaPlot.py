...
df = pd.read_csv("results/results1680826330.0254781.csv")
...
df.groupby(["Name", "Method"])["Accuracy"].mean().unstack().reindex(
    methods_order, axis=1
).plot(kind="bar", ax=ax, width=0.8)

print(
    df.groupby(["Name", "Method"])["Accuracy"]
    .mean()
    .unstack()
...
fig, ax = plt.subplots(figsize=(10, 6))
df.groupby(["Name", "Method"])["Time"].mean().unstack().reindex(
    methods_order, axis=1
).plot(kind="bar", ax=ax, width=0.8)
ax.set_xlabel("Method and Dataset Name")
ax.set_ylabel("Time")
...
df_pivot = df_mean.reset_index().pivot(
    index="Name", columns="Method", values="Accuracy"
)
