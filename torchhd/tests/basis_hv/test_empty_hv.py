...
class Testempty:
    ...
    def test_value(self, dtype, model):
        if not supported_dtype(dtype, model):
            with pytest.raises(ValueError):
                functional.empty(3, 26, model, dtype=dtype)

            return

        hv = functional.empty(8, 26, model, dtype=dtype)
    ...
    def test_device(self, dtype, model):
        ...
        hv = functional.empty(3, 52, model, device=device, dtype=dtype)
        assert hv.device == device

    def test_uses_default_dtype(self):
        hv = functional.empty(3, 52, torchhd.BSCTensor)
        assert hv.dtype == torch.bool

        torch.set_default_dtype(torch.float32)
        hv = functional.empty(3, 52, torchhd.MAPTensor)
        assert hv.dtype == torch.float32
        hv = functional.empty(3, 52, torchhd.HRRTensor)
        assert hv.dtype == torch.float32
        ...
        hv = functional.empty(3, 52, torchhd.FHRRTensor)
        assert hv.dtype == torch.complex64

    def test_requires_grad(self):
        hv = functional.empty(3, 52, torchhd.MAPTensor, requires_grad=True)
        assert hv.requires_grad == True

        hv = functional.empty(3, 52, torchhd.HRRTensor, requires_grad=True)
        assert hv.requires_grad == True

        hv = functional.empty(3, 52, torchhd.FHRRTensor, requires_grad=True)
        assert hv.requires_grad == True
