...
class TestRandom_hv:
    ...
    def test_shape(self, n, d, model):
        hv = functional.random_hv(n, d, model)
    ...
    def test_generator(self, model):
        ...
        hv1 = functional.random_hv(20, 10000, model, generator=generator)

        generator = torch.Generator()
        generator.manual_seed(seed)

        hv2 = functional.random_hv(20, 10000, model, generator=generator)
    ...
    def test_value(self, dtype, model):
        ...
        hv = functional.random_hv(8, 26000, model, dtype=dtype, generator=generator)
    ...
    def test_orthogonality(self, dtype, model):
        ...
        hv = functional.random_hv(100, 10000, model, dtype=dtype, generator=generator)
        sims = functional.cos_similarity(hv[0], hv[1:])
        assert torch.allclose(
    ...
    def test_uses_default_dtype(self):
        ...
        hv = functional.random_hv(3, 52, torchhd.MAP)
        assert hv.dtype == torch.float32
        hv = functional.random_hv(3, 52, torchhd.HRR)
        assert hv.dtype == torch.float32
        ...
        hv = functional.random_hv(3, 52, torchhd.FHRR)
        assert hv.dtype == torch.complex64

    def test_requires_grad(self):
        hv = functional.random_hv(3, 52, torchhd.MAP, requires_grad=True)
        assert hv.requires_grad == True

        hv = functional.random_hv(3, 52, torchhd.HRR, requires_grad=True)
        assert hv.requires_grad == True

        hv = functional.random_hv(3, 52, torchhd.FHRR, requires_grad=True)
        assert hv.requires_grad == True
