...
def train_quantHD(train_loader, device, encode, model, model_quantize, iterations, num_classes, lr=1, epsilon=0.01):
        ...
        for iter in range(iterations):
            ...
            for idx, (samples, labels) in enumerate(tqdm(train_loader, desc="Training")):
                ...
                if idx < validation_set:
                    ...
                    if idx == validation_set:
                        model.binarize_model(model_quantize)
                    outputs = model.quantized_similarity(samples_hv, model_quantize).float()
