...
def train_vanillaHD(train_loader, device, encode, model):
    ...
    for i in range(len(model.weight.data)):
        ...
        print(torchhd.functional.cosine_similarity(model.weight.data[i], model.weight_err.data[i]))
        m = torch.nonzero(abs(torch.sign(model.weight.data[i])-torch.sign(model.weight_err.data[i]))).squeeze(1)
        s = 1 - torchhd.functional.cosine_similarity(model.weight.data[i], model.weight_err.data[i])
