...
df = pd.read_csv("../results/final_results/dimensions_arenaaa.csv")
var = "Method"
...
mean_accuracy_by_dimension_and_method = df.groupby(["Dimensions", "Method"])["Accuracy"].mean()
var_accuracy_by_dimension_and_method = df.groupby(["Dimensions", "Method"])["Accuracy"].var()
...
#    lambda row: row.replace(row.max(), "bof(" + str(row.max()) + ")"), axis=1
#)
mean_accuracy_by_dimension_and_method = df.groupby(["Dimensions","Method"])['Accuracy'].mean().round(3)
...
var_accuracy_by_dimension_and_method = df.groupby(["Dimensions", "Method"])["Accuracy"].var().round(5)
...
train_time_by_dimension_and_method = df.groupby(["Dimensions", "Method"])["TrainTime"].var().round(3)

print(train_time_by_dimension_and_method)
order = ['add','adapt','online','adjust']
train_time_by_dimension_and_method = train_time_by_dimension_and_method.unstack('Method').reindex(columns=order)
...
test_time_by_dimension_and_method = df.groupby(["Dimensions", "Method"])["TestTime"].var().round(3)

print(test_time_by_dimension_and_method)
order = ['add','adapt','online','adjust']
test_time_by_dimension_and_method = test_time_by_dimension_and_method.unstack('Method').reindex(columns=order)
