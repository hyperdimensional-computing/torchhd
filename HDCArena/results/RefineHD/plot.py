...
file = (
    "/Users/verges/Documents/PhD/TorchHd/torchhd/HDCArena/results/RefineHD/online_dimensions_uci"
)
...
mean_of_encoding_train_time = (
    df.groupby(["name"])["accuracy"].mean().round(3).T
)
#print(mean_of_encoding_train_time.T)
mean_of_encoding_train_time2 = (
    df2.groupby(["name"])["accuracy"].mean().round(3).T
)

import numpy as np
print("MAAX",max(mean_of_encoding_train_time2.T - mean_of_encoding_train_time.T), np.argmax(mean_of_encoding_train_time2.T - mean_of_encoding_train_time.T))
print("MIIN",min(mean_of_encoding_train_time2.T - mean_of_encoding_train_time.T),np.argmin(mean_of_encoding_train_time2.T - mean_of_encoding_train_time.T))
