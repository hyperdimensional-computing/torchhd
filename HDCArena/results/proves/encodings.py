...
embeddings_order = ["bundle", "sequence", "ngram", "hashmap", "density", "flocet", "generic", "random", "sinusoid"]
...
if latex:
    ...
    if train_time:
        latex_table = mean_of_encoding_train_time.to_latex(index=False, caption='Encodings train time mean')
        print(latex_table)
    if test_time:
        latex_table = mean_of_encoding_test_time.to_latex(index=False, caption='Encodings test time mean')
        print(latex_table)
