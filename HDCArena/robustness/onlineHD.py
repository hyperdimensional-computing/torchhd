...
def test_onlineHD(
    ...
    model.normalize()
    acc = torchmetrics.Accuracy("multiclass", num_classes=model.out_features).to(device)
    accuracies = [
        torchmetrics.Accuracy("multiclass", num_classes=model.out_features).to(device)
